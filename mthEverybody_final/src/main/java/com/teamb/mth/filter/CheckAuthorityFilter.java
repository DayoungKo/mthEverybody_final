...
public class CheckAuthorityFilter implements Filter 
{
			...
			throws IOException, ServletException 
	{
		...
		//uri에서 customer, seller, admin 구분하여 권한 없으면 메인페이지로 리다이렉트
		...
		else if(uri.matches("/requestSellerForm"))
		{
			if(!member_code.equals("CUSTOMER"))  { res.sendRedirect(firsUrl+"/mainPage"); }
			else { chain.doFilter(request, response); }
		}
		else { chain.doFilter(request, response); }
	}
