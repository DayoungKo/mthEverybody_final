...
public class MemberController {
	...
	public String actionLoginForm(HttpSession session, HttpServletRequest request,
									Model model, MemberVO requestMemberVO,
									@RequestParam(value = "beforeUrl", required = false) String beforeUrl) 
	{
		...
		/* 로그인 후 이동할 페이지 처리 */
		// NULL처리
		if (beforeUrl == null || beforeUrl == "") { beforeUrl = "/mainPage"; }
		...
		String[] reservedUrlList = { "/message", "/writeReportForm", "/requestSellerForm", "/joinMemberForm", "/loginForm",
		...
		if (Arrays.asList(reservedUrlList).contains(requestUrl)) { requestUrl = "/mainPage"; }

		// requestUrl이 action* 매핑이면 mainPage로
		if (requestUrl.contains("/action")) { requestUrl = "/mainPage"; }
	...
	public String actionLogout(HttpSession session, HttpServletRequest request) 
	{
		...
		/* 예외처리 */
		// NULL처리
		if (requestUrl == null || requestUrl == "") { requestUrl = "/mainPage"; }
	...
	//임시 비밀번호 전송
	@RequestMapping("sendTempPassword")
	public String sendTempPassword(MemberVO requestMemberVO, Model model, HttpServletRequest request) throws UnsupportedEncodingException, MessagingException
	{
		//유효성 검사
		CheckRequestMemberInfo MemberData = new CheckRequestMemberInfo(requestMemberVO);
		
		if((MemberData.checkOnlyId() && MemberData.checkOnlyEmail()) == false)
		{
			model.addAttribute("message", "유효한 값을 입력해주세요.");
			model.addAttribute("page", "loginForm");

			return "/message";
		}
		
		//존재하는 아이디 또는 이메일인지 확인 후 메일 전송
		String member_email = requestMemberVO.getMember_email();
		
		MemberVO existMemberVOById = memberService.getMemberVOById(requestMemberVO);
		MemberVO existMemberVOByEmail = memberService.getMemberVOByEmail(member_email);
		String message;
		
		if(existMemberVOById == null)
		{
			message = "존재하지 않는 아이디 입니다.";
		}
		else if(existMemberVOByEmail == null)
		{
			message = "존재하지 않는 이메일 입니다.";
		}
		else if(!existMemberVOById.getMember_email().equals(member_email))
		{
		...
		{
			//임시 비밀번호 세팅
			String tempPassword = new TempKeyMaker().getKey(10, false);
			String member_idx = existMemberVOById.getMember_idx();
			
			MemberVO setMemberVO = new MemberVO();
			setMemberVO.setMember_idx(member_idx);
			setMemberVO.setMember_pw(tempPassword);
			
			//비밀번호 변경 후 메일 전송
			memberService.updateMemberPassword(setMemberVO);
			
			/* 메일 전송 */
			// 메일전송시 필요한 정보 설정
			MailHandler sendMail = new MailHandler(mailSender);
			String email = member_email;
			String nick = existMemberVOById.getMember_nick();
			String url = request.getRequestURL().toString().replaceAll(request.getServletPath(), "") + "/mainPage";

			// 메일 전송 세부 설정 및 스레드 생성
			ThreadSendMail threadSendMail = new ThreadSendMail(email, sendMail);
			threadSendMail.setMessageForNewPassword(nick, tempPassword, url);
	...
	{
		// 가입된 아이디 혹은 이메일인지 확인
		boolean checkUnValidMemberVO = memberService.checkValidJoinId(requestMemberVO);
		...
		else if (checkUnValidMemberVO == false)
		{
									...
									Model model, HttpServletRequest request) 
	{
		...
		String[] accessUrl = {"/loginForm", "/joinMemberForm", "/actionUpdateEmail"};
	...
	//이메일 수정
	@RequestMapping("actionUpdateEmail")
	public String actionUpdateEmail(MemberVO requestMemberVO, Model model, HttpServletRequest request)
									throws UnsupportedEncodingException, MessagingException
	{
		String member_idx = requestMemberVO.getMember_idx();
		boolean checkValidEmail = memberService.checkValidJoinEmail(requestMemberVO);
		
		//유효성 검사 및 가입한 메일인지 확인
		if(new CheckRequestMemberInfo(requestMemberVO).checkOnlyEmail() == false)
		{
			model.addAttribute("message", "잘못된 입력값입니다.");
			model.addAttribute("page", "emailConfirming?requestMemberIdx="+member_idx);
		...
		{
			//MemberVO, MemberMailVO 수정
			memberService.updateMemberEmail(requestMemberVO);
			memberMailService.resetCertifying(member_idx);
	...
	public String actionEmailConfirm(MemberMailVO requestMemberMailVO, Model model) 
	{
		...
		if (checkKeyResult == true) {
			...
			message = "잘못된 인증키 입니다. 인증키를 재전송해주시기 바랍니다.";
		}
			...
			throws UnsupportedEncodingException 
	{
		...
		String checkCompanynum = memberService.checkMemberCompanynum(requestMemberVO.getMember_companynum());
		
		// 유효성 검사
		if (new CheckRequestMemberInfo(requestMemberVO).checkReqSellerInfo() == false || checkCompanynum.equals("EXIST")) 
		{
			model.addAttribute("message", "잘못된 입력값입니다.");
			...
			@RequestParam(value = "requestPagingNums", required = false) Integer[] requestPagingNums) 
	{
		...
		if (requestPagingNums == null) {
			...
			for (int index = 0; index < requestPagingNums.length; index++) {
				if (requestPagingNums[index] == null || requestPagingNums[index] < 0) { requestPagingNums[index] = 0; }
			}
		}
	...
	public String actionUpdateMemberMyInfo(HttpSession session, MemberVO requestMemberVO, Model model) throws UnsupportedEncodingException 
	{
		// requestMemberVo 세팅
		...
		if (new CheckRequestMemberInfo(requestMemberVO).checkInfoForUpdate() == false) {
			...
			if (member_code.equals("SELLER") || member_code.equals("ADMIN")) {
				...
				memberService.updateMemberInfo(requestMemberVO);
			}
		}
