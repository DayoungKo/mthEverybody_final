...
import com.teamb.mth.util.CheckDataBytes;
...
public class ConvenController {
	...
	public String readConvenience(HttpSession session, Model model, @RequestParam("conven_idx") String conven_idx,
		...
		// 좋아요 여부, 횟수 prodData에 넣어주기
		for (ProdData prodData : prodDataList) 
		{
			// 로그인한 회원이 좋아요한 여부 check
			int itemLikeAllCount = itemLikeService.countItemLike(prodData.getItemVO().getItem_idx());
			boolean itemLikeCheckWhoseLogined = false;
			...
			if (user != null) {
				for (ItemLikeVO itemLikeVO : likeMemberList) {
					if (user.getMember_idx().equals(itemLikeVO.getMember_idx())) { itemLikeCheckWhoseLogined = true; }
				}
			}
			prodData.setItemLikeCheck(itemLikeCheckWhoseLogined);
			prodData.setItemLikeCount(itemLikeAllCount);
		}

		model.addAttribute("prodDataList", prodDataList);

		// 편의점 댓글 페이징 처리
		if (requestPagingNum == null || requestPagingNum < 0) { requestPagingNum = 0; }
		
		PagingData requestPagingData = new PagingData(convenReService.getConvenReListTotalCount(conven_idx), 5, requestPagingNum, 5);
		model.addAttribute("pagingData", requestPagingData);

		// 편의점 댓글리스트 가져오기
		ArrayList<ConvenReData> convenReDataList = convenReService.getConvenReListByConvenIdx(conven_idx, requestPagingData);
	...
	public String actionWriteConvenReContent(ConvenReVO requestConvenReVO, Integer requestPagingNum, Model model) throws UnsupportedEncodingException 
	{
		//bytes 크기 체크후 input
		boolean isUnderbytes = CheckDataBytes.checkUnderBytes(requestConvenReVO.getConven_re_content(), 300);
		String conven_idx = requestConvenReVO.getConven_idx();
		
		if(isUnderbytes == false) {
			model.addAttribute("message", "댓글 크기를 초과하였습니다.");
			model.addAttribute("page", "readConvenience?requestPagingNum=" + requestPagingNum + "&conven_idx=" + conven_idx);

			return "/message";
		}
		else { convenReService.writeConvenReContent(requestConvenReVO); }
	...
	public String actionUpdateConvenReContent(ConvenReVO requestConvenReVO, Integer requestPagingNum, Model model) throws UnsupportedEncodingException 
	{
		//bytes 크기 체크후 input
		boolean isUnderbytes = CheckDataBytes.checkUnderBytes(requestConvenReVO.getConven_re_content(), 300);
		String conven_idx = requestConvenReVO.getConven_idx();
		...
		else { convenReService.updateConvenReContent(requestConvenReVO); }
