...
public class ProdController {
			...
			@RequestParam(value = "searchWord", required = false) String searchWord) 
	{
		...
		if (searchOrder == null) { searchOrder = "prod_idx"; }
		...
		if (searchPrice == null) {
			...
			prodStockList = prodService.getProdListUpperPrice(searchData);	//5001 의미 묻기: 가격 5000원 이상인 것들
		} 
		else {
		...
		for (ProdData prodData : prodStockList) 
		{
			...
			{
				if (!itemVO.getItem_name().contains(searchWord)) { continue; }
			}
			...
			{
				if (!convenVO.getConven_name().contains(searchWord)) { continue; }
			}
