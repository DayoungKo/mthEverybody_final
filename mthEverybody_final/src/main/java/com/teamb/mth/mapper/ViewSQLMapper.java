...
public interface ViewSQLMapper 
{
	...
	@Select("SELECT * "
		  + "FROM(SELECT ROWNUM RNUM, FIR.* "
			   + "FROM (SELECT *  FROM VIEW_ALL_BOARD_MYINFO "
			   + "WHERE MEMBER_IDX=#{requestMemberVO.member_idx} "
			   + "ORDER BY WRITEDATE DESC)FIR"
			   + ")SEC "
		  + "WHERE SEC.RNUM BETWEEN #{requestPagingData.startNum} AND #{requestPagingData.endNum}")
	public ArrayList<AllBoardViewVO> selectAllBoardView(@Param("requestMemberVO")MemberVO requestMemberVO, 
